...
async function* exploreWalkAsync(dir, path8, followSymlinks, useStat, shouldSkip, strict) {
  let files = await readdir2(path8 + dir, strict);
...
var GDriveSink = class {
  ...
  async open(options) {
    ...
    const dir = await (0, import_promises3.mkdtemp)(import_node_path3.default.join(root, "qbx-gdrive-sink-"));
...
function buildConfig(flags) {
  ...
  return loadConfig(source, {
    localDir: import_node_path6.default.resolve("backups"),
    resourceDir: import_node_path6.default.resolve(__dirname, "..")
  });
}
async function commandRun(config) {
  ...
  let sink = new LocalFileSink(import_node_path6.default.resolve(config.localDir, names.zipName));
  if (config.mode === "s3" && isS3Configured(config.s3)) {
    ...
    sink = new S3Sink({
      client: s3Client,
      s3Key: key,
      tmpDir: import_node_path6.default.resolve(config.localDir, ".tmp"),
      keepLocalPath: config.keepLocal ? import_node_path6.default.resolve(config.localDir, names.zipName) : void 0
    });
    ...
    sink = new GDriveSink({
      ...
      tmpDir: import_node_path6.default.resolve(config.localDir, ".tmp"),
      keepLocalPath: config.keepLocal || config.gdrive.keepLocal ? import_node_path6.default.resolve(config.localDir, names.zipName) : void 0
    });
    ...
    sink = new DiscordSink({
      ...
      trigger: "cli",
      tmpDir: import_node_path6.default.resolve(config.localDir, ".tmp"),
      keepLocalPath: config.keepLocal || config.discord.keepLocal ? import_node_path6.default.resolve(config.localDir, names.zipName) : void 0
    });
  }
