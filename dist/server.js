...
async function pruneGDriveFolder(client, policy) {
  ...
  for (const file of gdriveFiles) {
    ...
    const ts = parseBackupTimestamp(file.name) ?? (file.createdTime ? new Date(file.createdTime).getTime() : 0);
    entries.push({
      id: file.id,
      name: file.name,
      timestamp: ts,
...
function createSpool3(target, options, maxBytes) {
  ...
  return {
    ...
    abort: async () => {
      ...
      counter.transform.destroy();
      file.destroy();
      await (0, import_promises6.rm)(target, { force: true });
    },
    failed
  };
}
