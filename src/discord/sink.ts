...
export class DiscordSink implements BackupSink {
  constructor(private readonly options: DiscordSinkOptions) {}

  async open(options: SinkOpenOptions): Promise<OpenSink> {
    const root = this.options.tmpDir;
    await mkdir(root, { recursive: true });
    const dir = await mkdtemp(path.join(root, "qbx-discord-sink-"));
    ...
    return {
      ...
      finish: async () => {
        try {
          ...
          // Stream the spool file to avoid loading large zips into memory
          const zipBlob = await openAsBlob(spoolPath, { type: "application/zip" });
          ...
          const locationTarget = uploadResult.threadId
            ? `discord://${uploadResult.channelId}/threads/${uploadResult.threadId}`
            : uploadResult.channelId
              ? `discord://${uploadResult.channelId}`
              : "discord://webhook";

          const finalLocation = uploadResult.fileAttached
            ? `${locationTarget} (${opts.fileName})`
            : `${locationTarget} (Embed-only summary: attachment skipped)`;

          return localCopy === null
            ? { ...result, location: finalLocation }
            : { ...result, location: `${finalLocation} (+ local: ${localCopy})` };
        } finally {
