...
import { GDriveError, type GDriveOAuthConfig } from './types';
...
describe('Google Drive REST Client', () => {
  ...
  it('initiates resumable upload and returns session URI', async () => {
    const mockFetch = mock(async (url: string | URL | Request, init?: RequestInit) => {
      ...
      if (urlStr.includes('/files?uploadType=resumable')) {
        ...
        return new Response('', {
          status: 200,
          headers: {
            Location: 'https://www.googleapis.com/upload/drive/v3/files?uploadType=resumable&upload_id=session_xyz',
          },
        });
      }
      return new Response('Not Found', { status: 404 });
    ...
    expect(sessionUri).toBe('https://www.googleapis.com/upload/drive/v3/files?uploadType=resumable&upload_id=session_xyz');
  });
      ...
      if (urlStr.includes('/drive/v3/files')) {
        ...
        if (listCallCount === 1) {
          return new Response(
            JSON.stringify({
              files: [
                { id: 'f1', name: 'qbx-2026-09-11_12-00-00Z.zip', size: '1000' },
              ],
              nextPageToken: 'page2',
            }),
            { status: 200, headers: { 'Content-Type': 'application/json' } }
        ...
        return new Response(
          JSON.stringify({
            files: [
              { id: 'f2', name: 'qbx-2026-09-10_12-00-00Z.zip', size: '2000' },
            ],
          }),
          { status: 200, headers: { 'Content-Type': 'application/json' } }
        );
  ...
  it('refreshes token on 401 Unauthorized and retries request', async () => {
    ...
    const mockFetch = mock(async (url: string | URL | Request) => {
      ...
      if (urlStr.includes('/about?fields=storageQuota')) {
        ...
        return new Response(
          JSON.stringify({ storageQuota: { limit: '1000' } }),
          { status: 200, headers: { 'Content-Type': 'application/json' } }
        );
      }
      return new Response('Not Found', { status: 404 });
